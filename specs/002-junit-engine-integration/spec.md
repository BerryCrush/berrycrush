...
**Created**: 2026-04-07  
**Status**: Draft  
**Input**: User description: "The library must provide custom JUnit engine, called `berrycrush`. The engine must resolve the `*.scenario` files by checking the `BerryCrushScenarios` annotation. And custom configuration can be taken from the `BerryCrushConfiguration` annotation. To maximize the interoperability to Java, the `sample` project must be rewritten in Java 21. `sample/petstore` project must implement API of the `petstore.yaml` with H2 database. Then its test module must test with the newly created JUnit engine, with `*.scenario` files. If needed, add custom binding to test."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Run Scenario Tests via JUnit Engine (Priority: P1)

As a developer using berrycrush, I want to run my BDD scenario files (`.scenario`) through JUnit 5 by annotating my test class with `@BerryCrushScenarios`, so that my scenario tests integrate seamlessly with my existing test infrastructure and IDE.
...
**Independent Test**: Can be fully tested by creating a test class annotated with `@BerryCrushScenarios(locations = "scenarios/")`, running it via JUnit Platform, and verifying that scenario files are discovered and executed with pass/fail results reported correctly.

**Acceptance Scenarios**:

1. **Given** a test class annotated with `@BerryCrushScenarios(locations = "scenarios/")`, **When** JUnit 5 discovers tests, **Then** all `.scenario` files in the specified location are discovered as test cases
...
4. **Given** a test class without `@BerryCrushScenarios`, **When** JUnit 5 runs, **Then** the berrycrush engine does not interfere with other tests

---

### User Story 2 - Configure Scenario Execution (Priority: P2)

As a developer, I want to configure the scenario execution behavior via `@BerryCrushConfiguration` annotation, so that I can customize bindings, timeouts, and other execution parameters per test class.
...
**Independent Test**: Can be tested by creating a test class with `@BerryCrushConfiguration` specifying custom bindings, running scenarios, and verifying the bindings are applied correctly.

**Acceptance Scenarios**:

1. **Given** a test class with `@BerryCrushConfiguration(bindings = MyBindings.class)`, **When** scenarios execute, **Then** custom bindings from `MyBindings` are used for scenario execution
2. **Given** a test class without `@BerryCrushConfiguration`, **When** scenarios execute, **Then** default configuration is applied
3. **Given** a test class with `@BerryCrushConfiguration`, **When** the configuration class is invalid or missing, **Then** a clear error message is reported at test discovery time

---

### User Story 3 - Integrate with Spring Boot Tests (Priority: P3)

As a developer using Spring Boot, I want to run berrycrush scenarios within a Spring Boot test context using `@SpringBootTest` and `@IncludeEngines("berrycrush")`, so that my API scenarios can run against a live Spring application context with full dependency injection.
...
**Independent Test**: Can be tested by creating a Spring Boot test class with `@SpringBootTest`, `@IncludeEngines("berrycrush")`, and `@BerryCrushScenarios`, running petstore API scenarios, and verifying HTTP calls reach the running Spring application.

**Acceptance Scenarios**:

1. **Given** a Spring Boot test class with `@SpringBootTest` and `@IncludeEngines("berrycrush")`, **When** tests run, **Then** the Spring application context is started before scenarios execute
...
As a developer evaluating berrycrush, I want a complete sample implementation of the petstore API in Java 21 with Spring Boot and H2, so that I can see how to integrate scenario testing into a real application.
...
3. **Given** the petstore sample, **When** I review the test classes, **Then** I can understand how to configure and use `@BerryCrushScenarios` and `@BerryCrushConfiguration`
...
**JUnit Engine (berrycrush/junit module)**:

- **FR-001**: System MUST provide a JUnit 5 TestEngine implementation registered with engine ID `berrycrush`
- **FR-002**: System MUST discover scenario files based on `@BerryCrushScenarios` annotation on test classes
- **FR-003**: System MUST support `locations` attribute in `@BerryCrushScenarios` to specify scenario file paths (relative to classpath)
...
- **FR-007**: System MUST provide `@BerryCrushConfiguration` annotation for custom configuration per test class
- **FR-008**: System MUST support `bindings` attribute in `@BerryCrushConfiguration` to specify custom binding classes
...
- **FR-014**: Sample project MUST include test classes using `@SpringBootTest` with `@IncludeEngines("berrycrush")`
...
- **BerryCrushScenarios**: Annotation marking a test class for scenario discovery with `locations` attribute
- **BerryCrushConfiguration**: Annotation providing configuration options including `bindings` for custom binding classes
...
- Scenario files follow the `.scenario` extension and the format defined by berrycrush core module
...
- The berrycrush core module provides the scenario parsing and execution capabilities that the JUnit engine will utilize
