...
class ErrorContextFormatter(
    ...
    internal fun formatAssertionException(exception: AssertionException): String =
        buildString {
            ...
            appendLine("  ${formatValue(exception.expected)}")
            appendLine()
            appendLine(colorize("Actual:", AnsiColors.RED))
            appendLine("  ${formatValue(exception.actual)}")

            // Show diff if both are strings
            if (exception.expected is String && exception.actual is String) {
                val diff = computeDiff(exception.expected, exception.actual)
                if (diff.isNotEmpty()) {
                    appendLine()
    ...
    private fun StringBuilder.appendRequest(
        ...
        if (config.includeRequestBody && request.body != null) {
            appendLine()
            appendTruncatedBody("Request Body", request.body, config.maxBodySize)
        }
    }
    ...
    private fun StringBuilder.appendResponse(
        ...
        if (config.includeResponseBody && response.body != null) {
            appendLine()
            appendTruncatedBody("Response Body", response.body, config.maxBodySize)
        }
    }
